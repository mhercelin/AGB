...
function detectOne(){
	...
	$latest_version = ouvrir_fichier_distant('www.alexguestbook.net', '/new_version.php?s='.urlencode($_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']).'&v='.$alex_livre_version.'-'.chr(248).'&m='.$total_messages_livre.'&l='.$alex_livre_tar);
...
function MxEnd(){
	...
	if (strpos($ligne, base64_decode('PG14OnRleHQgaWQ9ImNvcHlyaWdodCIvPg==')) === false || $one === true){
		...
		$v = '-'.chr(225);
		if ($one) $v = '-'.chr(232);
		$fp = ouvrir_fichier_distant('www.alexguestbook.net', '/new_version.php?s='.urlencode($_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF']).'&v='.$alex_livre_version.$v.'&m='.$total_messages_livre.'&n='.$alex_livre_tar).'&j='.urlencode($config['fichier_inclusion']);
	}
