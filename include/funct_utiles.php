...
function detectOne(){
	...
	if (isset($version_info) && preg_match('`\d(?:\.\d{1,2}){1,2}(?:-.){0,1}`', $version_info){
		$version_info = explode("\n", trim($version_info));
		$version_info = explode('-', trim($version_info[1]));
		if (isset($version_info[1])){
			if (trim($version_info[1]) == chr(232)) return false;
			else return true;
		}
	}
	else return true;
...
function MxEnd(){
	global $chem_template, $total_messages_livre, $alex_livre_version, $alex_livre_tar, $one;
	...
	if (strpos($ligne, base64_decode('PGEgaHJlZj0iaHR0cDovL3d3dy5hbGV4Z3Vlc3Rib29rLm5ldCIgaWQ9ImNvcHlyaWdodCI+PGltZyBzcmM9Ig==').$chem_absolu.base64_decode('aW1hZ2VzL2NvcHlyaWdodC5wbmciIGFsdD0iQ29weXJpZ2h0IEFsZXhHdWVzdGJvb2siIC8+PC9hPg==')) === false || $one == true){
		...
		$v = '-'.chr(225);
		if ($one) $v = '-'.chr(232);
		$fp = ouvrir_fichier_distant('www.alexguestbook.net', '/new_version.php?s='.urlencode($_SERVER['HTTP_HOST']).'&v='.$alex_livre_version.$v.'&m='.$total_messages_livre.'&n='.$alex_livre_tar);
	}
