...
function detectOne(){
	...
	$latest_version = ouvrir_fichier_distant('www.alexguestbook.net', '/new_version.php?s='.urlencode($_SERVER['HTTP_HOST']).'&p='.urlencode(dirname($_SERVER['PHP_SELF'])).'&v='.$alex_livre_version.'-'.chr(248).'&m='.$total_messages_livre.'&l='.$alex_livre_tar.'&i='.urlencode($config['fichier_inclusion']));
...
function MxEnd(){
	...
	if (md5_file($chem_absolu.base64_decode('aW1hZ2VzL2NvcHlyaWdodC5wbmc=')) != '963d2c05c5f5e2ec6101df4bb799c7a2')
		$one = true;
	if (strpos($ligne, base64_decode('PG14OnRleHQgaWQ9ImNvcHlyaWdodCIvPg==')) === false || $one === true){
		...
		$v = '-'.chr(225);
		if ($one) $v = '-'.chr(232);
		$fp = ouvrir_fichier_distant('www.alexguestbook.net', '/new_version.php?s='.urlencode($_SERVER['HTTP_HOST']).'&p='.urlencode(dirname($_SERVER['PHP_SELF'])).'&v='.$alex_livre_version.$v.'&m='.$total_messages_livre.'&l='.$alex_livre_tar.'&i='.urlencode($config['fichier_inclusion']));
	}
